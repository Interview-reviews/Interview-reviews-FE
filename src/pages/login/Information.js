      ...
      <div
        ...
        <form style={{ display: 'flex', flexDirection: 'column' }}>
          <div>
            ...
            <input value={school} onChange={e => setSchool(e.currentTarget.value)} placeholder="학교명" />
          </div>
          <div>
            <input value={major} onChange={e => setMajor(e.currentTarget.value)} placeholder="전공명" />
            <input value={grades} onChange={onlyNumber} maxLength={4} placeholder="소수점 둘째자리까지 학점입력" />
          </div>
          ...
          <div>
            ...
            <p style={{ color: 'gray', fontSize: '12px' }}>희망하는 직무 또는 현재 직무를 골라주세요.</p>
          </div>
          <label>어학</label>
          {language.map(info => {
            console.log(info);
          ...
          <div>
            <label>인턴 / 대외활동</label>
            <input onChange={e => setIntern(e.currentTarget.value)} placeholder={'횟수'} />
          </div>
          <div>
